...
Public Sub ResizePixBox(PicBox As Object, _
  ...
  Dim NewW As Long                                             'New Width
   ...
   If NewH > MaxHeight Then                 'picture is too large
      If NewH > NewW Then                                      'height is greater than width
         NewW = Fix((NewW / NewH) * MaxHeight) - 200                'rescale height
         NewH = MaxHeight - 200                                     'set max height
      End If
   End If
   
   If NewW > MaxWidth Then
      If NewW > NewH Then                                 'width is greater than height
         NewH = Fix((NewH / NewW) * MaxWidth) - 200                 'rescale width
         NewW = MaxWidth - 200                                      'set max Width
        ' Debug.Print "Width>"
       End If                                                   'image is square
    End If
   ...
   If Center = True Then                                        'center picture
      PicTop = (PicBox.Height / 2) - (NewH / 2) - 40
      PicLeft = (PicBox.Width / 2) - (NewW / 2) - 40
